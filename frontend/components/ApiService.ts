...
async function apiFetch(path: string, options?: RequestInit) {
  if (!BASE_URL) throw new Error('Backend URL is not configured');
...
export const api = {
  ...
  getBook: (abbrev: string) => apiFetch(`/books/${encodeURIComponent(abbrev)}`),
  getChapter: (abbrev: string, num: number) => apiFetch(`/chapters/${encodeURIComponent(abbrev)}/${num}`),
  ...
  // Paths
  getPaths: (userId: string) => apiFetch(`/paths?user_id=${encodeURIComponent(userId)}`),
  ...
  // Notes
  getNotes: (userId: string) => apiFetch(`/notes?user_id=${encodeURIComponent(userId)}`),
  ...
  // Highlights
  getHighlights: (userId: string) => apiFetch(`/highlights?user_id=${encodeURIComponent(userId)}`),
